...
def run_php_server(port):
    ...
    print(Fore.GREEN+" [+] "+Fore.LIGHTCYAN_EX+f"LocalHost                    => "+Fore.YELLOW+Back.BLACK+f"http://localhost:{port}")
    print(Fore.GREEN+"\n [+] "+Fore.LIGHTCYAN_EX+f"Jalankan NGROK Di Port {port}  => "+Fore.YELLOW+Back.BLACK+f"ngrok http {port}\n"+Style.RESET_ALL)
